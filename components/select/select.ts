...
let optionsTemplate = `
    ...
    <div *ngIf="optionsOpened && options && options.length > 0 && itemObjects[0].hasChildren()" class="md2-select-menu">
      <div class="md2-optgroup" *ngFor="#c of options; #index=index">
        <label>{{c.text}}</label>
        <div class="md2-option" *ngFor="#o of c.children" [class.active]="isActive(o)" (mouseenter)="selectActive(o)" (click)="selectMatch(o, $event)" [ngClass]="{'active': isActive(o)}">
          <div class="md2-text" [innerHtml]="o.text | hightlight:inputValue"></div>
        </div>
      </div>
    </div>
...
export class Select {
    ...
    private focusToInput(value: string = '') {
        setTimeout(() => {
            let el = this.element.nativeElement.querySelector('div.md2-select-value-container > input');
            if (el) {
                el.focus();
                el.value = value;
            }
    ...
    public inputEvent(e: any, isUpMode: boolean = false) {
        ...
        if (!isUpMode && e.keyCode === 8) {
            let el: any = this.element.nativeElement
                .querySelector('div.md2-select-value-container > input');

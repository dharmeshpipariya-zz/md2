  ...
  template: `
    <div class="md2-tags-layout">
      <div class="md2-tags-container">
        <span *ngIf="selectedValue.length < 1" class="md2-tags-placeholder">{{placeholder}}</span>
...
export class Md2Tags implements AfterContentInit, ControlValueAccessor {
  ...
  private onClick(e: any) {
    ...
    this.tempList = this._items.map((item: any) => new Item(item, this.itemText));
    this.list = this.tempList.filter((option: Item) => (!this.selectedValue.find((o: Item) => option.text === o.text)));
  ...
  private getOffSideClickHandler(context: any) {
    return function (e: any) {
      let elem = e.target;
      do {
        if (elem === context.element.nativeElement && elem.className && elem.className.indexOf('md2-tabs') < 0) { return; }
  ...
  private removeTag(tag, event) {
    let index = this.selectedValue.findIndex(item => item.text === tag.text);
    this.selectedValue.splice(index, 1);
  }
...
class Behavior {
  ...
  public ensureHighlightVisible(listMap: Map<string, number> = null) {
    let container = this.actor.element.nativeElement.querySelector('.md2-tabs-suggestions');
